...
| Sign-up flow + onboarding tutorial — invite email with a sign-up link → details form → user profile; Google OAuth consent/publishing + email provider (product-plan §4); first-run tutorial | 9 | 6 | backend | in-progress (PR A #279 invites, PR C #280 tenancy, PR B #281 sign-up, PR D #282 account; next: tutorial) |
...
- **PR D shipped (#282):** `/account` (profile name/phone via `PATCH /me`;
