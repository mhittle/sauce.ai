...
| Sign-up flow + onboarding tutorial — invite email with a sign-up link → details form → user profile; Google OAuth consent/publishing + email provider (product-plan §4); first-run tutorial | 9 | 6 | backend | in-progress (PR A #279 invites, PR C #TBD tenancy; next B sign-up, D account, tutorial) |
...
- **PR C shipped (#TBD):** migration 0014 (`orgs`, `org_id` on users/
  ...
  controls. MA-014 (make Rida a platform admin). Next: PR B sign-up.
