...
-- 3-bullet TL;DR generated by Haiku from the extracted body (separate
-- isolated pass in classify_pending). Kept out of articles/article_features
-- so the hot feed/ranking queries stay lean; CASCADE-pruned with the article.
...
-- Breaking-news email alerts. `user_alert_prefs` carries the opt-in
-- toggle + per-user daily cap counter + dedicated unsubscribe token
-- (independent of the daily digest's token so the two channels can be
-- managed separately). `breaking_news_alerts` is the per-event log
-- with UNIQUE on `story_id` — each story alerts at most once and
-- already-rejected stories are not re-judged on the next 15-min tick.
-- Both tables are NOT BUG-007 class: only /account/settings,
-- /account/alerts/unsubscribe/*, and the breaking_alerts cron touch
-- them, and all three callers tolerate the tables being missing.
-- See migrations/2026-05-22-breaking-alerts.sql.
...
-- /claim: one cached health-headline reality check per feed article, pasted
-- URL, or pasted text. Reads tolerate the table being absent (the route
-- degrades to an un-persisted card); NOT BUG-007 class.
