...
-- ---------------------------------------------------------------------------
-- Solicitations (PRD PlanHub track): public procurement / bid-board records,
-- often with plan/spec PDFs attached. Bid-side analogue of permits/projects.
-- ---------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS solicitations (
    id                  BIGSERIAL PRIMARY KEY,
    source_type         TEXT NOT NULL,                 -- samgov|esbd-tx|vbs-fl|gpr-ga|bidnet|demandstar
    source_id           TEXT NOT NULL,                 -- platform's solicitation id
    title               TEXT,
    agency              TEXT,
    description         TEXT,
    naics               TEXT,
    ...
    posted_date         DATE,
    due_date            DATE,                          -- bid / response deadline
    status              TEXT,
    source_url          TEXT,
    geom                geometry(Point, 4326),
    project_id          BIGINT REFERENCES projects(id),-- linked once matched to a permit project
    raw_payload         JSONB NOT NULL DEFAULT '{}'::jsonb,
    first_seen          TIMESTAMPTZ NOT NULL DEFAULT now(),
    last_seen           TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- Casework/cabinetry classification (added post-v1; idempotent for existing DBs).
...
CREATE TABLE IF NOT EXISTS solicitation_documents (
    ...
    name                TEXT,
    url                 TEXT NOT NULL,
    doc_type            TEXT NOT NULL DEFAULT 'attachment', -- plans|spec|addendum|attachment|other
    bytes               BIGINT,
    storage_key         TEXT,                          -- object-store key once downloaded
    text_extract        TEXT,                          -- parsed text for the LLM scope pass
